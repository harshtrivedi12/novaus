...
function EducationPage() {
    ...
    const conversations = useSelector((state) => state.skillTestQuestionSlice.skillTestQuestions.questions)
    const skill_assessment_id = useSelector((state) => state.skillTestQuestionSlice.skillTestQuestions.skill_assessment_id)
    const body = useSelector((state) => state.skillTestQuestionSlice.skillTestQuestions)
...
console.log(currentConversationObj);
        ...
        {`${time % 60}`.padStart(2, 0)}
                                             ...
                                             <ul
                                                 ...
                                                 {conversations.map((item, index) => (
                                                    console.log(conversations),
                                                     <li key={index}>
                                                         <Link to={"#"} className={`conversation-item ${index === currentConversation ? "active" : ""}`} onClick={() => setCurrentConversation(index)}>
                                                             <h6 className="mb-0">{index +1}: {item.question}</h6>
                                ...
                                <div className="col-xl-8 col-lg-7 m-b30">
                                    <div className="job-bx submit-resume " style={{padding:"100px 35px"}}>
                                        <h4>{currentConversation+1}: {currentConversationName}</h4>
                                        <Form>
    ...
    {currentConversationObj.options.map((item, index) => (
        <div className="bg-light p-3" style={{display: "block", marginTop: "10px"}}>
            <Form.Check
                type="radio"
                key={index}
                label={item}
                checked={conversations[currentConversation].user_answer === item}
                className="p-10"
                name={`question-${currentConversation}`}
                onClick={() => {
                    console.log(`Selected item ${item} for question ${currentConversation}`);
                    dispatch(setUserAnswer({ index: currentConversation, answer: item }))
                }}
            />
        </div>
    ))}
                                           ...
                                           {currentConversation !== 0? (  
                                                <button  className="site-button" onClick={(e) => {
                                                      e.preventDefault()
                                                    setCurrentConversation(currentConversation - 1 )}}> Previous</button>
                                                    ) : null}

{conversationsLen - 1 !== currentConversation ? null :  (  
                                                <button  className="site-button" onClick={(e) => {
                                                    e.preventDefault()
                                                    submitTest()}}> Submit</button>
                                                    ) }
                                                    {conversationsLen - 1 !== currentConversation ? (  
                                                <button  className="site-button" onClick={(e) => {
                                                      e.preventDefault()
                                                    setCurrentConversation(currentConversation + 1 )}}> Next</button>
                                                    ) : null}
                                                </div>
                                        </Form>
                                    </div>
